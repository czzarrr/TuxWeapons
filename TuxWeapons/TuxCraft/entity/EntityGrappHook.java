...
public class EntityGrappHook extends Entity implements IProjectile {
	...
	// public EntityPlayer owner = (EntityPlayer) owner;
	...
	public EntityGrappHook(World world, double par2, double par4, double par6) {
	...
	public EntityGrappHook(World world, EntityLivingBase player,
			EntityLivingBase par3EntityLiving, float par4, float par5) {

		super(world);
		this.renderDistanceWeight = 10.0D;
		this.owner = (EntityPlayer) player;
		...
		double var6 = par3EntityLiving.posX - player.posX;
		double var8 = par3EntityLiving.posY + par3EntityLiving.getEyeHeight()
				- 0.699999988079071D - this.posY;
		double var10 = par3EntityLiving.posZ - player.posZ;
		...
		if (var12 >= 1.0E-7D) {
			...
			this.setLocationAndAngles(player.posX + var16, this.posY,
					player.posZ + var18, var14, var15);
			this.yOffset = 0.0F;
	...
	public EntityGrappHook(World world, EntityLivingBase player, float par3,
			int par4, Item par5Item) {
		...
		par1NBTTagCompound.setCompoundTag("Stack",
				new ItemStack(par5Item).writeToNBT(new NBTTagCompound()));
	}
	...
	public void onUpdate() {
		...
		if (owner.inventory.getCurrentItem() != null
				&& owner.inventory.getCurrentItem().getItem() != this.item) {
			recall();
		}
				...
				if (var4.entityHit != null) {
					...
					if (var4.entityHit.attackEntityFrom(var21, var23)) {
						if (var4.entityHit instanceof EntityLiving) {
							...
							if (this.owner != null) {
								EnchantmentThorns.func_92096_a(this.owner,
										var24, this.rand);
							}

							if (this.owner != null
									&& var4.entityHit != this.owner
									&& var4.entityHit instanceof EntityPlayer
									&& this.owner instanceof EntityPlayerMP) {
								((EntityPlayerMP) this.owner).playerNetServerHandler
		...
		if (Math.abs(this.posX - owner.posX) > 30) {
			this.recall();

			return;
		}

		if (Math.abs(this.posY - owner.posY) > 30) {
			this.recall();

			return;
		}

		if (Math.abs(this.posZ - owner.posZ) > 30) {
			this.recall();
	...
	public void writeEntityToNBT(NBTTagCompound par1NBTTagCompound) {
		...
		par1NBTTagCompound.setByte("shake", (byte) this.BoltShake);
	...
	public void readEntityFromNBT(NBTTagCompound par1NBTTagCompound) {
		...
		this.BoltShake = par1NBTTagCompound.getByte("shake") & 255;
	...
	public int recall() {
		...
		this.kill();
		this.owner = null;
		itemGrappHook.grappHookEntity = null;

		return 1;
	}

	public static ItemStack getStack(EntityGrappHook hook) {
