...
public class RenderSpear extends Render {
	...
	public void renderSpear(EntitySpear spear, double par2, double par4,
			double par6, float par8, float par9) {

		ItemStack stack = spear.getStack(spear);
		...
		if (stack != null) {
			...
			GL11.glRotatef(spear.prevRotationYaw
					+ (spear.rotationYaw - spear.prevRotationYaw) * par9
					- 90.0F, 0.0F, 1.0F, 0.0F);
			GL11.glRotatef(
					(spear.prevRotationPitch + (spear.rotationPitch - spear.prevRotationPitch)
							* par9) - 45, 0.0F, 0.0F, 1.0F);

			GL11.glTranslatef(-0.1F, 0.0F, 0.0F);

			float var21 = spear.arrowShake - par9;
			...
			for (int k = 0; k < stack.getItem().getRenderPasses(
				...
				this.renderDroppedItem(spear, icon, 1, par9, f5 * f8, f4 * f8,
						f6 * f8, k);
			}
	...
	private void renderDroppedItem(EntitySpear par1EntityItem, Icon par2Icon,
		...
		if (par2Icon == null) {
			...
			ResourceLocation resourcelocation = texturemanager
					.func_130087_a(par1EntityItem.getStack(par1EntityItem)
							.getItemSpriteNumber());
		...
		float f12 = 0.0625F;
		f11 = 0.021875F;
		ItemStack itemstack = par1EntityItem.getStack(par1EntityItem);
		int j = 1;
		byte b0 = 1;
	...
	public void doRender(Entity par1Entity, double par2, double par4,
		...
		if (par1Entity.worldObj.isRemote)
			this.renderSpear((EntitySpear) par1Entity, par2, par4, par6, par8,
					par9);
	}
	...
	protected ResourceLocation func_110775_a(Entity entity) {
		...
		return this.renderManager.renderEngine.func_130087_a(spear.getStack(
				spear).getItemSpriteNumber());
	}
}
